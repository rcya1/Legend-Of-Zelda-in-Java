package utility;

//Stores all data for the game
public class Data
{
	public static int swordLevel = 0;
	public static int rupees = 0;

	public static int shieldLevel = 0;

	public static int boomerangLevel = 1;

	public static boolean hasBombs = false;
	public static int numberOfBombs = 0;

	public static boolean hasBow = false;
	public static int arrowLevel = 0;

	public static int candleLevel = 0;

	public static boolean hasWhistle = false;

	public static boolean hasBait = false;

	//0 - Nothing, cannot buy, 1 - Letter, can buy, 2 - Health, 3 - Better Health
	public static int potionLevel = 0;

	public static boolean hasMagicWand = false;
}
